...
export class StoreApiHelper {
  ...
  async getInventory(): Promise<any> {
    ...
    console.log('Inventory Response:', body);
    return body;
  }
  ...
  async getOrderById(orderId: number): Promise<any> {
    const response = await this.request.get(`${this.basePath}/order/${orderId}`);
    expect(response.ok()).toBeTruthy();
    const body = await response.json();
  ...
  async deleteOrderById(orderId: number): Promise<void> {
    const response = await this.request.delete(`${this.basePath}/order/${orderId}`);
